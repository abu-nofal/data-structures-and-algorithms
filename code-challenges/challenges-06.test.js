...
const getNames = (arr) => {
  ...
  let newArray=arr.map(value=>{
    return value.name.split("").reverse().join("").split(" ").reverse().join(" ") // i taket as it from the web it should revers the elemnt insid the array 
  })
  return newArray
};

//   arr.split( " " ).map(  //split into words and iterate via map
...
const appendFirstToLast = (arr) => {
  // Solution code here...
  arr.push(arr[0])// here we can modifi arr used the push method 
  return arr;
...
const addBirthYearProperty = (obj, year) => {
  // Solution code here...
  obj.yearBorn=year;// add new prop to the obj 
  return obj
};
...
const setStatusAsAuthor = (people) => {
  ...
  for (let i = 0; i < people.length; i++) {
    ...
    people[i].isAuthor=true;// here we should pass into the object iside the array 
  }
...
const append = (arr1, arr2) => {
  ...
  // arr1.push.apply(arr1, arr2);// if we need it in the same array 
  arr1.push(...arr2); // or we can also used this methode to solve it (The Spread Operator)
  // let newArray= arr1.concat(arr2)// if we need the value in new array 
  return arr1;
